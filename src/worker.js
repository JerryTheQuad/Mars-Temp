...
async function buildTemperaturePayload() {
  ...
  return {
    ...
    next_allowed_refresh_at: nextAllowedRefreshAt.toISOString(),
...
async function handleApiTemperature(request) {
  const cache = caches.default;
  const cacheKey = new Request(new URL("/api/temperature", request.url).toString());
...
export default {
  async fetch(request, env) {
    ...
    if (request.method === "GET" && url.pathname === "/api/temperature") {
      return handleApiTemperature(request);
    }
